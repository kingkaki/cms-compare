...
$.fn.pageinfo=function(){
    ...
    $(this).find("img,[data-original]").each(function(index, el) {
    	...
    	if(img_url){
    		...
    		if(imgval.length && (imgval[0].indexOf('&met-id')>=0 || imgval[0].indexOf('?met-id')>=0)) $(this).attr({'met-id':imgval[0].split('=')[1],'met-table':imgval[1].split('=')[1],'met-field':imgval[2].split('=')[1]});
    	}
    });
    // 图标内容转换可视化信息
